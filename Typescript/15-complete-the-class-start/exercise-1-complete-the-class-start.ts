...
class Currency {
  ...
  constructor(name: string, code: string, symbol: symbol) {
    this.name = name;
    this.code = code;
    this.symbol = symbol;
  ...
  describe(): void {
    ...
    description += `and uses the symbol ${this.symbol.toString()}.`;
...
const currencyNaira = new Currency("Naira", "NGN", Symbol("₦"));
...
const currencyUsDollar = new Currency("United States dollar", "USD", Symbol("$"));
